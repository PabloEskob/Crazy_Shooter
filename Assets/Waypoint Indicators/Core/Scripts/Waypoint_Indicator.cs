...
public class Waypoint_Indicator : MonoBehaviour
{
    ...
    private GameObject[]
        mainCameraObjs; //This is temporarily used to see if more than one game object is taggd with this name
    ...
    private GameObject[]
        distCalTargets; //This is temporarily used to see if more than one game object is taggd with this name

    private GameObject
        distCalTarget; //The GAME OBJECT this game object uses to calculate the distance from (defualt is camera)

    public string
        distCalTargetTag =
            "MainCamera"; //The GAME OBJECT TAG NAME this game object uses to calculate the distance from (defualt is camera)
    ...
    [Header("Tracking Types")] [Space(10)] [Header("*** Custom Editor Missing! See documentation. ***")] [Space(10)]
    public bool
        enableStandardTracking = false; //Tracks over objects in view and to the edge of the screen when out of view.

    public bool
        enableCenteredTracking =
            false; //Tracks objects along a circular perimeter originating from the center of the screen.

    [Header("Parent")] public bool showBoundaryBox = true;
    ...
    [Header("Sprite Global")] public bool enableSprite = false;
    ...
    [Range(0, 360)] public float onScreenSpriteRotation = 0f;
    ...
    [Range(0, 360)] public float offScreenSpriteRotation = 0f;
    ...
    [Header("Game Object Global")] public bool enableGameObject = false;
    public int gameObjectDepth;
    public bool offScreenObjectRotates = false;

    //ON-SCREEN GAME OBJECT
    [Header("Object On-Screen")] public GameObject onScreenGameObject;
    ...
    [Range(0, 360)] public float onScreenGameObjectRotation = 0f;
    ...
    [Header("Object Off-Screen")] public GameObject offScreenGameObject;
    ...
    [Range(0, 360)] public float offScreenGameObjectRotation = 0f;
    ...
    [Header("Text Global")] public bool enableText = true;
    ...
    [Header("Centered Sprite On-Screen")] public bool enableCenteredSprite;
    ...
    [Range(0, 360)] public float onScreenCenteredSpriteRotation = 0f;
    ...
    [Header("Centered Prefab On-Screen")] public bool enableCenteredPrefab;
    ...
    [Range(0, 360)] public float onScreenCenteredPrefabRotation = 0f;
    ...
    [Header("Radius")]
    public bool
        enableRadiusGizmo; //A circular wireframe tool for aligning crescent-shaped icons.  Scene View only. Must have Gizmos and 2D enabled.
    ...
    private Vector3
        objectInWorldScreenPos; //This is the WorldToScreen x,y,z of the object this script sits on (Standard Tracking)
    ...
    private RawImage
        gameObjectIndicatorChildImg; //The image that controls the sprite that sits inside the game object (index 0)
    ...
    private RawImage
        centeredPrefabIndicatorChildImg; //The image that controls the sprite that sits inside the game object (index 0)
    ...
    void CheckForCamera()
    {
        ...
        if (camera_tag_name == "" || camera_tag_name == null || camera_tag_name == " ")
        {
            ...
            if (!multiCam)
            {
                Debug.LogError("Camera tag name field is blank! See waypoint_indicator.cs on Game Object: \"" +
                               gameObject.name + "\"");
            }
        ...
        else //Tag name has a value now lets check to see if that value exsists as a tag somewhere
        {
            ...
            else
            {
                ...
                if (mainCameraObjs.Length > 1)
                {
                    ...
                    Debug.LogError("The Camera tag name \"" + camera_tag_name +
                                   "\" on waypoint_indicator.cs on Game Object: \"" + gameObject.name +
                                   "\" is already being used in your scene " + mainCameraObjs.Length +
                                   " times: (see below)");
                ...
                if (mainCameraObjs.Length < 1)
                {
                    ...
                    if (!multiCam)
                    {
                        Debug.LogError("You tagged a Camera \"" + camera_tag_name +
                                       "\" in your scene, but I can't find it! See waypoint_indicator.cs on Game Object: \"" +
                                       gameObject.name + "\"");
                    }
    ...
    void CheckForCanvas()
    {
        ...
        {
            //Tag name is blank please add a tag
            Debug.LogError("Canvas tag name field is blank! See waypoint_indicator.cs on Game Object: \"" +
                           gameObject.name + "\"");
            mainCanvas = null;
        }
        else //Tag name has a value now lets check to see if that value exsists as a tag somewhere
        {
            ...
            else
            {
                ...
                {
                    //There are more than one object of this tag name and here is a list (please make sure just one is labeled)
                    Debug.LogError("The Canvas tag name \"" + canvas_tag_name +
                                   "\" on waypoint_indicator.cs on Game Object: \"" + gameObject.name +
                                   "\" is already being used in your scene " + mainCanvasObjs.Length +
                                   " times: (see below)");
                ...
                {
                    //There are no objects in the scene that match this tag, please add one in
                    Debug.LogError("You tagged a Canvas \"" + canvas_tag_name +
                                   "\" in your scene, but I can't find it! See waypoint_indicator.cs on Game Object: \"" +
                                   gameObject.name + "\"");
                    mainCanvas = null;
                }
        ...
        if (mainCamera && mainCanvas)
        {
            ...
            if (enableStandardTracking)
            {
                ...
                if (onScreenGameObject != null ||
                    offScreenGameObject != null) //At least one icon was defined for one slot
                {
                    //Check to see if either on or off-screen has been left empty and assign the sprite from the state that has been defined 
                    if (onScreenGameObject == null)
                    {
            ...
            if (enableStandardTracking)
            {
                ...
                if (iStandardTrackingEnabled == 0)
                {
                    ...
                    else
                    {
                        if (enableGameObject)
                        {
                            Debug.LogWarning("Game Object Indicators are ENABLED, but NOT ASSIGNED on: " +
                                             gameObject.name +
                                             "\nAssign a Game Object to either On/Off Screen Game Object fields.");
                        }
                    }
                ...
                waypointDistInt =
                    (int)waypointDist; //We convert this for the text readout so users dont see: Distance 23.547893m
                ...
                if (wpParentGameObject && wpParentRectTransform != null)
                {
                    ...
                    if (mainCamera != null)
                    {
                        ...
                        switch (mainCanvas.renderMode)
                        {
                            ...
                            case RenderMode.ScreenSpaceCamera:
                                objectInWorldScreenPos =
                                    mainCamera.WorldToScreenPoint(transform
                                        .position); //0,0 bottom left | 0,1 top left | 1,1 top right | 1,0 bottom right
                                wpParentRectTransform.position = mainCamera.ScreenToWorldPoint(
                                    new Vector3(objectInWorldScreenPos.x, objectInWorldScreenPos.y,
                                        mainCanvas.planeDistance));
                                break;
                            ...
                            case RenderMode.WorldSpace:
                                uiOffset = new Vector3((float)mainCanvasRect.sizeDelta.x / 2f,
                                    (float)mainCanvasRect.sizeDelta.y / 2f, 0);

                                objectInViewportPos = mainCamera.WorldToViewportPoint(transform.position);
                                proportionalPosition = new Vector3(objectInViewportPos.x * mainCanvasRect.sizeDelta.x,
                                    objectInViewportPos.y * mainCanvasRect.sizeDelta.y, 0);
                    ...
                    switch (mainCanvas.renderMode)
                    {
                        ...
                        case RenderMode.ScreenSpaceOverlay:
                            ...
                            onScreenSnapOffset_X = 22 - (UnityEngine.Screen.width * .024f);
                            ...
                            //Onscreen
                            if (wpParentPos.z > 0f &&
                                (wpParentPos.x / canvasScaleFactor) > (wpParentRectTransform.sizeDelta.x / 2) &&
                                wpParentPos.x < (UnityEngine.Screen.width - (wpParentRectTransform.sizeDelta.x / 2)) +
                                onScreenSnapOffset_X &&
                                wpParentPos.y / canvasScaleFactor > (wpParentRectTransform.sizeDelta.y / 2) &&
                                wpParentPos.y < (UnityEngine.Screen.height - (wpParentRectTransform.sizeDelta.y / 2)) +
                                onScreenSnapOffset_Y)
                            {
                                //Debug.Log("On");
                                if (iScreenCheck == 1)
                                {
                            ...
                            else //(Offscreen)
                            {
                                ...
                                screenCenter = new Vector3(UnityEngine.Screen.width / 2, UnityEngine.Screen.height / 2,
                                    0f);
                        ...
                        case RenderMode.ScreenSpaceCamera:
                            ...
                            if (objectInWorldScreenPos.z > 0f &&
                                (objectInWorldScreenPos.x / canvasScaleFactor) >
                                (wpParentRectTransform.sizeDelta.x / 2) && objectInWorldScreenPos.x <
                                (mainCamera.pixelWidth - (wpParentRectTransform.sizeDelta.x / 2)) +
                                onScreenSnapOffset_X &&
                                objectInWorldScreenPos.y / canvasScaleFactor >
                                (wpParentRectTransform.sizeDelta.y / 2) && objectInWorldScreenPos.y <
                                (mainCamera.pixelHeight - (wpParentRectTransform.sizeDelta.y / 2)) +
                                onScreenSnapOffset_Y)
                            {
                                if (iScreenCheck == 1)
                                {
                            ...
                            else //Out of camera viewport
                            {
                                ...
                                else if (objectInWorldScreenPos.x <
                                         -screenBounds.x) //Out of bounds! Must be on the left
                                {
                                    //left
                                    ScreenEdgeDetectLeft();
                        ...
                        case RenderMode.WorldSpace:
                            ...
                            if (objectInViewportPos.z > 0f &&
                                proportionalPosition.x > (wpParentRectTransform.sizeDelta.x / 2) &&
                                proportionalPosition.x <
                                (mainCanvasRect.sizeDelta.x - (wpParentRectTransform.sizeDelta.x / 2)) +
                                onScreenSnapOffset_X &&
                                proportionalPosition.y > (wpParentRectTransform.sizeDelta.y / 2) &&
                                proportionalPosition.y <
                                (mainCanvasRect.sizeDelta.y - (wpParentRectTransform.sizeDelta.y / 2)) +
                                onScreenSnapOffset_Y)
                            {
                                if (iScreenCheck == 1)
                                {
                            ...
                            else //Out of camera viewport
                            {
                                ...
                                //Find center of screen
                                screenCenter = new Vector3(mainCanvasRect.rect.width, mainCanvasRect.rect.height, 0f) /
                                               2;
                                //Set 0,0 DEAD CENTER from lower left
                    ...
                    if (spriteIndicatorCreated && spriteIndicatorRect != null)
                    {
                        ...
                        if (waypointDist < displayRangeMax && waypointDist > (displayRangeMin + 1)) //ICON IN RANGE
                        {
                            ...
                            if (parentOnScreen) //ICON ON-SCREEN
                            {
                                ...
                                spriteIndicatorRect.transform.localRotation =
                                    Quaternion.Euler(0f, 0f, onScreenSpriteRotation);
                                ...
                                else //SHOW
                                {
                                    ...
                                    {
                                        spriteIndicatorImage.rectTransform.localScale = new Vector3(newOnScreenSize.x,
                                            newOnScreenSize.y, Sprite_Z_Scale);
                                    }
                                }
                            }
                            ...
                            if (!parentOnScreen)
                            {
                                ...
                                if (offScreenSpriteRotates)
                                {
                                    ...
                                    spriteIndicatorRect.transform.localRotation = Quaternion.Euler(0f, 0f,
                                        angle * Mathf.Rad2Deg + offScreenSpriteRotation);
                                }
                                else
                                {
                                    //spriteIndicatorRect.transform.eulerAngles = new Vector3(0f, 0f, offScreenSpriteRotation);
                                    spriteIndicatorRect.transform.localRotation =
                                        Quaternion.Euler(0f, 0f, offScreenSpriteRotation);
                                }
                                ...
                                else //SHOW
                                {
                                    ...
                                    {
                                        spriteIndicatorImage.rectTransform.localScale = new Vector3(newOffScreenSize.x,
                                            newOffScreenSize.y, Sprite_Z_Scale);
                                    }
                                }
                            }
                        }
                        ...
                        if (waypointDist < displayRangeMax && waypointDist > (displayRangeMin + 1)) //PREFAB IN RANGE
                        {
                            ...
                            if (parentOnScreen) //ICON ON-SCREEN
                            {
                                ...
                                if (gameObjectIndicatorOnOffScreenStatus == 0)
                                {
                                    ...
                                    gameObjectIndicatorChildGameObject =
                                        gameObjectIndicator.transform.GetChild(0).gameObject;
                                    gameObjectIndicatorChildRect =
                                        gameObjectIndicatorChildGameObject.GetComponent<RectTransform>();
                                ...
                                gameObjectIndicator.transform.localRotation =
                                    Quaternion.Euler(0f, 0f, onScreenGameObjectRotation);
                                ...
                                else //Show
                                {
                                    ...
                                    if (onScreenGameObjectScaleWithRange)
                                    {
                                        ScaleGameObjectWithRange(onScreenGameObjectSize,
                                            reverseOnScreenGameObjectScaleWithRange);
                                    }
                                    else //Use user Scale
                                    {
                            ...
                            if (!parentOnScreen) //ICON OFF-SCREEN
                            {
                                ...
                                if (offScreenObjectRotates)
                                {
                                    ...
                                    gameObjectIndicator.transform.localRotation = Quaternion.Euler(0, 0,
                                        angle * Mathf.Rad2Deg + offScreenGameObjectRotation);
                                }
                                else
                                {
                                    //gameObjectIndicator.transform.eulerAngles = new Vector3(0f, 0f, offScreenGameObjectRotation);
                                    gameObjectIndicator.transform.localRotation =
                                        Quaternion.Euler(0, 0, offScreenGameObjectRotation);
                                }
                                ...
                                else //Show
                                {
                                    ...
                                    if (offScreenGameObjectScaleWithRange)
                                    {
                                        ScaleGameObjectWithRange(offScreenGameObjectSize,
                                            reverseOffScreenGameObjectScaleWithRange);
                                    }
                                    else //Use user Scale
                                    {
                        ...
                        if (waypointDist < displayRangeMax && waypointDist > (displayRangeMin + 1)) //TEXT IN RANGE
                        {
                            ...
                            if (!parentOnScreen)
                            {
                                ...
                                if (topEdgeDetected)
                                {
                                    ...
                                    {
                                        textGameObject.transform.localPosition =
                                            new Vector2(offScreenTextOffset.x, -edgeDetectOffset.y);
                                        textField.alignment = TextAlignmentOptions.Center;
                                    }
                                    else
                                    {
                                ...
                                if (rightEdgeDetected)
                                {
                                    ...
                                    {
                                        textGameObject.transform.localPosition =
                                            new Vector2(-edgeDetectOffset.x, offScreenTextOffset.y);
                                        textField.alignment = TextAlignmentOptions.Right;
                                    }
                                    else
                                    {
            ...
            if (enableCenteredTracking)
            {
                ...
                if (wpCenteredParentGameObject && wpCenteredParentRectTransform != null)
                {
                    ...
                    if (mainCamera != null)
                    {
                        ...
                        switch (mainCanvas.renderMode)
                        {
                            ...
                            case RenderMode.ScreenSpaceCamera:
                                objectInWorldScreenPosCentered =
                                    mainCamera.WorldToScreenPoint(transform
                                        .position); //0,0 bottom left | 0,1 top left | 1,1 top right | 1,0 bottom right
                                wpCenteredParentRectTransform.position = mainCamera.ScreenToWorldPoint(
                                    new Vector3(objectInWorldScreenPosCentered.x, objectInWorldScreenPosCentered.y,
                                        mainCanvas.planeDistance));
                                break;
                            ...
                            case RenderMode.WorldSpace:
                                uiOffsetCentered = new Vector3((float)mainCanvasRect.sizeDelta.x / 2f,
                                    (float)mainCanvasRect.sizeDelta.y / 2f, 0);

                                objectInWorldScreenPosCentered = mainCamera.WorldToViewportPoint(transform.position);
                                proportionalPositionCentered =
                                    new Vector3(objectInWorldScreenPosCentered.x * mainCanvasRect.sizeDelta.x,
                                        objectInWorldScreenPosCentered.y * mainCanvasRect.sizeDelta.y, 0);

                                // Set the position and remove the screen offset
                                wpCenteredParentRectTransform.localPosition =
                                    proportionalPositionCentered - uiOffsetCentered;
                    ...
                    switch (mainCanvas.renderMode)
                    {
                        ...
                        case RenderMode.ScreenSpaceOverlay:
                            ...
                            if (wpCenteredParentPos.z > 0f &&
                                wpCenteredParentPos.x > wpCenteredParentRectTransform.sizeDelta.x &&
                                wpCenteredParentPos.x < UnityEngine.Screen.width &&
                                wpCenteredParentPos.y > wpCenteredParentRectTransform.sizeDelta.x &&
                                wpCenteredParentPos.y < UnityEngine.Screen.height)
                            {
                                //On screen
                            ...
                            //Find center of screen
                            screenCenteredCenter =
                                new Vector3(UnityEngine.Screen.width, UnityEngine.Screen.height, 0f) / 2;
                            //Set 0,0 DEAD CENTER from lower left
                            ...
                            if (cosCentered > 0f)
                            {
                                ...
                                wpCenteredParentPos = new Vector3(-screenBoundsCentered.y / mCentered,
                                    screenBoundsCentered.y, 0f);
                            }
                            else
                            {
                            ...
                            if (wpCenteredParentPos.x > screenBoundsCentered.x) //Out of bounds! Must be on the right
                            {
                                ...
                                wpCenteredParentPos = new Vector3(screenBoundsCentered.x,
                                    -screenBoundsCentered.x * mCentered, 0f);
                            }
                            else if (wpCenteredParentPos.x <
                                     -screenBoundsCentered.x) //Out of bounds! Must be on the left
                            {
                                //right
                                ScreenEdgeDetectRight(); //This toggles screen edge detect bools for future use
                                wpCenteredParentPos = new Vector3(-screenBoundsCentered.x,
                                    screenBoundsCentered.x * mCentered, 0);
                            }
                        ...
                        case RenderMode.ScreenSpaceCamera:
                            ...
                            //Find angleCentered from center of screen to mouse pos
                            angleCentered = Mathf.Atan2(objectInWorldScreenPosCentered.y,
                                objectInWorldScreenPosCentered.x);
                            angleCentered -= 90f * Mathf.Deg2Rad;
                            ...
                            if (cosCentered > 0f)
                            {
                                //up
                                ScreenEdgeDetectTop(); //This toggles screen edge detect bools for future use
                                objectInWorldScreenPosCentered = new Vector3(-screenBoundsCentered.y / mCentered,
                                    screenBoundsCentered.y, 0f);
                            }
                            else
                            {
                            ...
                            //If out of bounds, get point on appropriate side
                            if (objectInWorldScreenPosCentered.x >
                                screenBoundsCentered.x) //Out of bounds! Must be on the right
                            {
                                //left
                                ScreenEdgeDetectLeft(); //This toggles screen edge detect bools for future use
                                objectInWorldScreenPosCentered = new Vector3(screenBoundsCentered.x,
                                    -screenBoundsCentered.x * mCentered, 0f);
                            }
                            else if (objectInWorldScreenPosCentered.x <
                                     -screenBoundsCentered.x) //Out of bounds! Must be on the left
                            {
                                //right
                                ScreenEdgeDetectRight(); //This toggles screen edge detect bools for future use
                        ...
                        case RenderMode.WorldSpace:
                            ...
                            //Find center of screen
                            screenCenteredCenter =
                                new Vector3(mainCanvasRect.sizeDelta.x, mainCanvasRect.sizeDelta.y, 0f) / 2;
                            //Set 0,0 DEAD CENTER from lower left
                            ...
                            if (cosCentered > 0f)
                            {
                                //up
                                ScreenEdgeDetectTop(); //This toggles screen edge detect bools for future use
                                proportionalPositionCentered = new Vector3(-screenBoundsCentered.y / mCentered,
                                    screenBoundsCentered.y, 0f);
                            }
                            else
                            {
                            ...
                            //If out of bounds, get point on appropriate side
                            if (proportionalPositionCentered.x >
                                screenBoundsCentered.x) //Out of bounds! Must be on the right
                            {
                                //left
                                ScreenEdgeDetectLeft(); //This toggles screen edge detect bools for future use
                                proportionalPositionCentered = new Vector3(screenBoundsCentered.x,
                                    -screenBoundsCentered.x * mCentered, 0f);
                            }
                            else if (proportionalPositionCentered.x <
                                     -screenBoundsCentered.x) //Out of bounds! Must be on the left
                            {
                                //right
                                ScreenEdgeDetectRight(); //This toggles screen edge detect bools for future use
                    ...
                    wpCenteredParentRectTransform.transform.localRotation =
                        Quaternion.Euler(0f, 0f, angleCentered * Mathf.Rad2Deg);
                ...
                if (enableCenteredSprite)
                {
                    ...
                    if (waypointIconCenteredCreated && iconCentered != null)
                    {
                        ...
                        if (waypointCenteredDist < onScreenCenteredRangeMax &&
                            waypointCenteredDist > onScreenCenteredRangeMin) //ICON IN RANGE
                        {
                            //Sprite
                            ...
                            iconCentered.transform.localRotation =
                                Quaternion.Euler(0f, 0f, onScreenCenteredSpriteRotation);
                            ...
                            {
                                iconCenteredImage.rectTransform.localScale = new Vector2(newOnScreenCenteredSize.x,
                                    newOnScreenCenteredSize.y);
                            }
                ...
                if (enableCenteredPrefab)
                {
                    ...
                    if (centeredPrefabIndicatorCreated && centeredPrefabIndicatorChildRect != null)
                    {
                            ...
                            waypointCenteredDist > onScreenCenteredRangeMin) //ICON IN RANGE
                        {
                            ...
                            centeredPrefabIndicator.transform.localRotation =
                                Quaternion.Euler(0f, 0f, onScreenCenteredPrefabRotation);
                            ...
                            //Size - User Defined
                            centeredPrefabIndicator.transform.localScale = new Vector3(onScreenCenteredPrefabSize,
                                onScreenCenteredPrefabSize, 0f);
                            ...
                            if (!centeredParentOnScreen) //OFF-SCREEN
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        ScaleCenteredPrefabWithRange(onScreenCenteredPrefabSize,
                                            onScreenPrefabCenteredScaleReverse);
                                    }
                                    else //Use user Scale
                                    {
                                        centeredPrefabIndicator.transform.localScale =
                                            new Vector3(onScreenCenteredPrefabSize, onScreenCenteredPrefabSize, 0f);
                                    }
                                }
                            }
                            else //ON-SCREEN
    ...
    void InstantiateWaypointParent()
    {
        if (mainCanvas != null)
        {
            ...
            wpParentRectTransform.position =
                new Vector3(-1000f, -1000f,
                    0f); //This make sure the blinking box doesn't appear in bottom left at spawn
            ...
            switch (mainCanvas.renderMode)
            {
                ...
                case RenderMode.ScreenSpaceOverlay:
                    if (wpParentPos.z > 0f &&
                        wpParentPos.x > (wpParentRectTransform.sizeDelta.x / 2) && wpParentPos.x <
                        UnityEngine.Screen.width - (wpParentRectTransform.sizeDelta.x / 2) &&
                        wpParentPos.y > (wpParentRectTransform.sizeDelta.x / 2) && wpParentPos.y <
                        UnityEngine.Screen.height - (wpParentRectTransform.sizeDelta.x / 2))
                    {
                        //On screen
                        if (iScreenCheck == 1)
                        {
                ...
                case RenderMode.ScreenSpaceCamera:
                    if (objectInWorldScreenPos.z > 0f &&
                        (objectInWorldScreenPos.x / canvasScaleFactor) > (wpParentRectTransform.sizeDelta.x / 2) &&
                        objectInWorldScreenPos.x < (mainCamera.pixelWidth - (wpParentRectTransform.sizeDelta.x / 2)) &&
                        objectInWorldScreenPos.y / canvasScaleFactor > (wpParentRectTransform.sizeDelta.y / 2) &&
                        objectInWorldScreenPos.y < (mainCamera.pixelHeight - (wpParentRectTransform.sizeDelta.y / 2)))
                    {
                        if (iScreenCheck == 1)
                        {
                            //Debug.Log("On Screen - Camera!");
                ...
                case RenderMode.WorldSpace:
                    if (objectInViewportPos.z > 0f &&
                        proportionalPosition.x > (wpParentRectTransform.sizeDelta.x / 2) && proportionalPosition.x <
                        (mainCanvasRect.sizeDelta.x - (wpParentRectTransform.sizeDelta.x / 2)) &&
                        proportionalPosition.y > (wpParentRectTransform.sizeDelta.y / 2) && proportionalPosition.y <
                        (mainCanvasRect.sizeDelta.y - (wpParentRectTransform.sizeDelta.y / 2)))
                    {
                        if (iScreenCheck == 1)
                        {
    ...
    void InstantiateWaypointIcon()
    {
        if (onScreenSprite != null && offScreenSprite != null) //Make sure BOTH icons are defined before making
        {
            ...
            spriteIndicatorImage.sprite = onScreenSprite;
            spriteIndicatorImage.rectTransform.localScale =
                new Vector3(newOnScreenSize.x, newOnScreenSize.y, Sprite_Z_Scale); //user set scale
    ...
    void ScaleSpriteWithRange(Vector2 OnOffScreenSize,
        bool reverseScalingSprite) //This should take onScreenSpriteSize and offScreenSpriteSize arguments
    {
        //Formula
        //1. Get displayRangeDifference (displayRangeMax - displayRangeMin)
        displayRangeDifference = displayRangeMax - displayRangeMin;
    ...
    void ScaleGameObjectWithRange(float OnOffScreenSize,
        bool reverseScalingGameObject) //This should take onScreenSpriteSize and offScreenSpriteSize arguments
    {
        //Formula
        //1. Get displayRangeDifference (displayRangeMax - displayRangeMin)
        displayRangeDifference = displayRangeMax - displayRangeMin;
        ...
        if (waypointDist < displayRangeMax && waypointDist > displayRangeMin)
        {
            ...
            {
                //Gets smaller as you get away from it and larger as you get closer
                scaleValueGameObjectX =
                    OnOffScreenSize * (1 - ((waypointDist - displayRangeMin) / displayRangeDifference));
                scaleValueGameObjectY =
                    OnOffScreenSize * (1 - ((waypointDist - displayRangeMin) / displayRangeDifference));
            }

            gameObjectIndicator.transform.localScale =
                new Vector3(scaleValueGameObjectX, scaleValueGameObjectY, Prefab_Z_Scale);
        }
        else //Waypoint is beyond the player preset, so set scale and alpha to 0
        {
            //Scale set to 0
    ...
    void InstantiateCenteredWaypointParent()
    {
        if (mainCanvas != null)
        {
            ...
            wpCenteredParentRectTransform.position =
                new Vector3(-1000f, -1000f,
                    0f); //This make sure the blinking box doesn't appear in bottom left at spawn
    ...
    void InstantiateCenteredWaypointIcon()
    {
        if (onScreenCenteredSprite != null)
        {
            ...
            iconCenteredImage.sprite = onScreenCenteredSprite;
            iconCenteredImage.rectTransform.localScale =
                new Vector2(newOnScreenCenteredSize.x, newOnScreenCenteredSize.y); //user set scale
    ...
    void InstantiateCenteredWaypointPrefab()
    {
        ...
        {
            centeredPrefabIndicator = Instantiate(onScreenCenteredPrefab, wpCenteredParentPos,
                Quaternion.Euler(0, 0, angle * Mathf.Rad2Deg)) as GameObject;
    ...
    void FadeWithRangeCentered()
    {
        ...
        if (waypointCenteredDist < onScreenCenteredRangeMax && waypointCenteredDist > onScreenCenteredRangeMin)
        {
            ...
            iconCenteredAlphaValue = 1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                          onScreenCenteredDisplayRangeDifference);
    ...
    void FadeCenteredPrefabWithRange()
    {
        ...
        {
            //Alpha in real time based off dist
            centeredPrefabIndicatorCanvasGroup.alpha = 1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                            onScreenCenteredDisplayRangeDifference);
        }
        else //Waypoint is beyond the player preset, so set scale
        {
    ...
    void ScaleWithRangeCentered(Vector2 OnOffScreenSize,
        bool reverseScalingSprite) //This should take onScreenCenteredSpriteSize and offScreenSize arguments
    {
        //Formula
        //1. Get displayRangeDifference (displayRangeMax - displayRangeMin)
        onScreenCenteredDisplayRangeDifference = onScreenCenteredRangeMax - onScreenCenteredRangeMin;
        ...
        if (waypointCenteredDist < onScreenCenteredRangeMax && waypointCenteredDist > onScreenCenteredRangeMin)
        {
            ...
            {
                //Gets larger as you get away from it and smaller as you get closer
                centeredScaleValueX = OnOffScreenSize.x * ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                           onScreenCenteredDisplayRangeDifference);
                centeredScaleValueY = OnOffScreenSize.y * ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                           onScreenCenteredDisplayRangeDifference);
            }
            else
            {
                //Gets smaller as you get away from it and larger as you get closer
                centeredScaleValueX = OnOffScreenSize.x * (1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                                onScreenCenteredDisplayRangeDifference));
                centeredScaleValueY = OnOffScreenSize.y * (1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                                onScreenCenteredDisplayRangeDifference));
            }
    ...
    void ScaleCenteredPrefabWithRange(float OnOffScreenSize, bool reverseScalingPrefab)
    {
        ...
        if (waypointCenteredDist < onScreenCenteredRangeMax && waypointCenteredDist > onScreenCenteredRangeMin)
        {
            ...
            {
                //Gets larger as you get away from it and smaller as you get closer
                centeredScaleValueX = OnOffScreenSize * ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                         onScreenCenteredDisplayRangeDifference);
                centeredScaleValueY = OnOffScreenSize * ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                         onScreenCenteredDisplayRangeDifference);
            }
            else
            {
                //Gets smaller as you get away from it and larger as you get closer
                centeredScaleValueX = OnOffScreenSize * (1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                              onScreenCenteredDisplayRangeDifference));
                centeredScaleValueY = OnOffScreenSize * (1 - ((waypointCenteredDist - onScreenCenteredRangeMin) /
                                                              onScreenCenteredDisplayRangeDifference));
            }
    ...
    void CalculateDistance()
    {
        ...
        if (!distanceTargetDefined)
        {
            ...
            {
                //Tag name is blank please add a tag
                Debug.LogWarning("Distance Calculation Tag was left blank on Game Object: \"" + gameObject.name +
                                 "\". Using the Camera tagged: \"" + camera_tag_name +
            ...
            else //Tag name has a value now lets check to see if that value exsists as a tag somewhere
            {
                ...
                {
                    //There are more than one object of this tag name and here is a list (please make sure just one is labeled)
                    Debug.LogWarning("The tag name \"" + distCalTargetTag +
                                     "\" on waypoint_indicator.cs on Game Object: \"" + gameObject.name +
                                     "\" is already being used in your scene " + distCalTargets.Length +
                                     " times: (see below)");
                    ...
                    Debug.LogWarning("Using the Camera tagged: \"" + camera_tag_name +
                                     "\" for distance calculations by default.");
